...
/**
 * @brief Determine the quantized value for a given quantization level.
 *
 * @param quant_level   The quantization level to use.
 * @param value         The value to convert; must be in 0-255 range.
 *
 * @return The value returned is quantized but still in the 0-255 range.
 */
static inline int quant_unquant_color(
	quant_method quant_level,
	int value
) {
	return color_quant_unquant_tables[quant_level - QUANT_6][value];
}
 ...
 *
 * @param quant_level   The quantization level to use.
 * @param value         The value to convert; will be clamped to the 0-255 range.
 *
 * @return The value returned is quantized but still in the 0-255 range.
 */
static inline vint4 quant_unquant_color3(
	quant_method quant_level,
	vint4 value
) {
	return vint4(
		color_quant_unquant_tables[quant_level - QUANT_6][value.lane<0>()],
		color_quant_unquant_tables[quant_level - QUANT_6][value.lane<1>()],
		color_quant_unquant_tables[quant_level - QUANT_6][value.lane<2>()],
		0.0f);
}
...
static void quantize_rgb(
	...
	color0 = (color0 * scale) + 0.5f;
	color1 = (color1 * scale) + 0.5f;

	vfloat4 inc_diff(0.2f);
	...
	do
	{
		vint4 color0i_uq = clamp(0, 255, float_to_int(color0));
		vint4 color1i_uq = clamp(0, 255, float_to_int(color1));

		color0i = quant_unquant_color3(quant_level, color0i_uq);
		color1i = quant_unquant_color3(quant_level, color1i_uq);
...
static bool try_quantize_rgb_blue_contract(
	...
	// Apply inverse blue-contraction; on overflow contraction cannot be used
...
static bool try_quantize_rgb_delta_blue_contract(
	...
	// Switch around endpoint colors at the start for sake of sanity
	float scale = 1.0f / 257.0f;
	color0 = color0 * scale;
	color1 = color1 * scale;
	...
	// Check that the sum of the encoded offsets is positive, else encoding fails
	vint4 color1du = color1de & 0x7F;
	vmask4 lane_mask = (color1du & 0x40) != 0;
	vint4 offset = select(vint4::zero(), vint4(0x80), lane_mask);
	color1du = color1du - offset;
...
static bool try_quantize_alpha_delta(
	...
	a0a <<= 1;
	int a0b = a0a & 0xFF;
	int a0be = quant_unquant_color(quant_level, a0b);
	a0b = a0be | (a0a & 0x100);
	...
	int a1de = quant_unquant_color(quant_level, a1d);
	if ((a1d ^ a1de) & 0xC0)
	{
		return false;
	}
...
static bool try_quantize_luminance_alpha_delta(
	...
	int l0be = quant_unquant_color(quant_level, l0b);
	int a0be = quant_unquant_color(quant_level, a0b);
	...
	int l1de = quant_unquant_color(quant_level, l1d);
	int a1de = quant_unquant_color(quant_level, a1d);
	if ((l1d ^ l1de) & 0xC0)
	{
		return false;
...
static void quantize_rgbs(
	...
	int ri = quant_unquant_color(quant_level, astc::flt2int_rtn(r));
	int gi = quant_unquant_color(quant_level, astc::flt2int_rtn(g));
	int bi = quant_unquant_color(quant_level, astc::flt2int_rtn(b));
	...
	output[3] = quant_unquant_color(quant_level, scale_idx);
}
...
static void quantize_rgbs_alpha(
	...
	output[4] = quant_unquant_color(quant_level, astc::flt2int_rtn(a0));
	output[5] = quant_unquant_color(quant_level, astc::flt2int_rtn(a1));
...
static void quantize_luminance(
	...
	output[0] = quant_unquant_color(quant_level, astc::flt2int_rtn(lum0));
	output[1] = quant_unquant_color(quant_level, astc::flt2int_rtn(lum1));
}
...
static void quantize_luminance_alpha(
	...
	output[0] = quant_unquant_color(quant_level, astc::flt2int_rtn(lum0));
	output[1] = quant_unquant_color(quant_level, astc::flt2int_rtn(lum1));
	output[2] = quant_unquant_color(quant_level, astc::flt2int_rtn(a0));
	output[3] = quant_unquant_color(quant_level, astc::flt2int_rtn(a1));
}
...
static inline void quantize_and_unquantize_retain_top_two_bits(
	...
	do
	{
		uquantval = quant_unquant_color(quant_level, value);
...
static void quantize_hdr_rgb(
	...
	for (int mode = 7; mode >= 0; mode--)
	{
		...
		int a_quantval = quant_unquant_color(quant_level, a_lowbits);
	...
	for (int i = 0; i < 4; i++)
	{
		...
		output[i] = quant_unquant_color(quant_level, idx);
	}
...
static void quantize_hdr_luminance_large_range(
	...
	// OK; encode
	output[0] = quant_unquant_color(quant_level, v0);
	output[1] = quant_unquant_color(quant_level, v1);
}
...
static bool try_quantize_hdr_luminance_small_range(
	...
	v0 = lowval & 0x7F;
	v0e = quant_unquant_color(quant_level, v0);
	v0d = v0e;
	...
	if (v0d < 0x80)
	{
		...
		if (diffval >= 0 && diffval <= 15)
		{
			...
			v1e = quant_unquant_color(quant_level, v1);
			v1d = v1e;
			if ((v1d & 0xF0) == (v1 & 0xF0))
			{
...
static void quantize_hdr_alpha(
	...
	for (int i = 2; i >= 0; i--)
	{
		...
		v6 = (val0 & 0x7F) | ((i & 1) << 7);
		v6e = quant_unquant_color(quant_level, v6);
		...
		v7e = quant_unquant_color(quant_level, v7);

		static const int testbits[3] { 0xE0, 0xF0, 0xF8 };
		if ((v7 ^ v7e) & testbits[i])
		{
			continue;
		}
	...
	output[0] = quant_unquant_color(quant_level, v6);
	output[1] = quant_unquant_color(quant_level, v7);
...
uint8_t pack_color_endpoints(
	...
	// Ensure colors are in a valid range for the encoding
	color0 = clamp(0.0f, 65535.0f, color0);
	color1 = clamp(0.0f, 65535.0f, color1);
