...
void compute_avgs_and_dirs_4_comp(
	...
	for (int partition = 0; partition < partition_count; partition++)
	{
		...
		// TODO: Can we compute average when choosing partitionings to save rescan?
		...
		vfloat4 average = base_sum / vfloat4(static_cast<float>(texel_count));
...
void compute_avgs_and_dirs_3_comp(
	...
	for (unsigned int partition = 0; partition < partition_count; partition++)
	{
		...
		{
			unsigned int iwt = texel_indexes[i];
			base_sum += vfloat4(data_vr[iwt], data_vg[iwt], data_vb[iwt], 0.0f);
		}
...
void compute_error_squared_rgba(
	...
	for (unsigned int partition = 0; partition < partition_count; partition++)
	{
		...
		for (unsigned int i = 0; i < texel_count; i += ASTCENC_SIMD_WIDTH)
		{
			...
			vfloat uncor_err = (uncor_dist0 * uncor_dist0)
			                 + (uncor_dist1 * uncor_dist1)
			                 + (uncor_dist2 * uncor_dist2)
			                 + (uncor_dist3 * uncor_dist3);
			...
			vfloat samec_err = (samec_dist0 * samec_dist0)
			                 + (samec_dist1 * samec_dist1)
			                 + (samec_dist2 * samec_dist2)
			                 + (samec_dist3 * samec_dist3);
